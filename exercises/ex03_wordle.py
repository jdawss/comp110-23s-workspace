...
# Initialize the variable indx to 0
indx: int = 0
...
def emojified(user_guess: str, secret_wrd: str) -> str:
    ...
    while indx < len(user_guess):
        ...
        if user_guess[indx] == secret_wrd[indx]:
            ...
            other = 0
            there = False
            # Loop through each character in the secret word to see if the character in the user's guess is present
            while other < len(secret_wrd) and not there:
                if secret_wrd[other] == user_guess[indx]:
                    in_word = True
                    there = True
                other += 1
            if in_word:
...
def input_guess(expected_length: int) -> str:
    ...
    while True:
        ...
        if len(run) == expected_length:
            ...
            run = input(f"That wasn't {expected_length} chars! Try again: ")
